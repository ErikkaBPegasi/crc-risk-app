...
# Inputs
# Fecha de nacimiento con selector de calendario (inicialmente vacío)
dob = st.date_input(
    "Fecha de nacimiento",
    value=None,
...
# Compute outputs when all required inputs exist
...
- Alternativa: TSOMFi cada 2 años o TSOMFg bienal."
    ...
    # 3. Polyp history
        ...
        if tmp_advanced:
            ...
            st.markdown("- Colonoscopia: control en 3 años.
- FIT/FOBT anual o bienal entre controles.")
        elif tmp_serrated:
            st.warning("**Riesgo Alto** (Poliposis serrada)")
            st.markdown("- Colonoscopia: vigilancia cada 3–5 años.
- Evaluación genética.")
        else:
            st.info("**Riesgo Intermedio** (Historial de pólipos)")
            st.markdown("- Colonoscopia: control en 5 años.
...
- Colonoscopia Virtual (VCC) cada 5 años (alternativa no invasiva)"
    ...
    # 7. >75 años
        ...
        st.warning(">75 años: evaluar caso a caso, sin tamizaje rutinario.")
