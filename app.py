...
ibd = st.checkbox("Enfermedad intestinal crónica (Crohn o colitis ulcerativa)")
hered = st.checkbox("Síndrome hereditario (p.ej. Lynch)")
hamart = st.checkbox("Síndrome de pólipos hereditario (Peutz‑Jeghers, Cowden)")
fap = st.checkbox("Poliposis adenomatosa familiar (PAF)")
fasha = st.checkbox("Poliposis familiar atenuada (PAFA)")
serrated_synd = st.checkbox("Síndrome de poliposis serrada (múltiples pólipos serrados)")

# 2. Antecedentes familiares
st.markdown("**2. Antecedentes familiares**")
family_crc = st.checkbox("Familiar de primer grado con cáncer colorrectal")
family_before_60 = False
if family_crc:
    family_before_60 = st.checkbox("¿Diagnóstico antes de 60 años?")

# 3. Historial de pólipos en colon o recto
st.markdown("**3. Historial de pólipos**")
polyp10 = st.checkbox("Pólipos detectados en los últimos 10 años")
advanced_poly = False
serrated = False
if polyp10:
    advanced_poly = st.checkbox("Pólipo grande (>1 cm) o de alto riesgo")
    serrated = st.checkbox("Pólipo serrado observado")

# 4. Síntomas actuales
st.markdown("**4. Síntomas actuales**")
symptoms = st.checkbox("Sangrado rectal, cambio de hábito intestinal o pérdida de peso inexplicada")
